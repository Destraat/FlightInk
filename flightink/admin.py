...
<div class="card"><h2>Live preview</h2><img src="/preview.png?x={{ range(1,999999)|random }}" onerror="this.style.display='none'"><form method="post" action="/action/preview"><button>Create new preview</button></form></div>
...
<label>DISPLAY_BACKEND</label><select name="DISPLAY_BACKEND"><option value="preview" {% if config.get('DISPLAY_BACKEND')=='preview' %}selected{% endif %}>preview</option><option value="waveshare" {% if config.get('DISPLAY_BACKEND')=='waveshare' %}selected{% endif %}>waveshare</option></select><p><button>Save</button></p></form></div></section><aside>
